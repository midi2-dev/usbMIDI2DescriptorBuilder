...
console.log("uint8_t const desc_fs_configuration[] = {")
console.log(arrayToHex(descriptor,', '));
console.log("};")

let gtbarray= [];
config.endpoints.map((ep,idx)=>{
    gtbarray.push(`gtb${idx}`);
    console.log(`uint8_t const gtb${idx}[] = {`)
    console.log(arrayToHex(ep.gtbDescriptor,', '));
    console.log("};")
});
console.log(`uint8_t const gtbLengths[] = {${config.endpoints.map(e=>e.gtbDescriptor.length).join(',')}};`)


console.log(`uint8_t const epInterface[] = {${config.endpoints.map(e=>e.interfaceId).join(',')}};`)


console.log(`uint8_t const *group_descr[] = {${gtbarray.join(',')}};`)

console.log("char const* string_desc_arr [] = {")
console.log(`"${stringIdx.join('", "')}"`)
console.log("};")

console.log(`uint8_t const string_desc_arr_length = ${stringIdx.length-1};`)
